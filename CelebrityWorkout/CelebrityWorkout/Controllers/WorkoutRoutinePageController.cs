...
{
    public class WorkoutRoutinePageController : Controller
    {
        private readonly IWorkoutRoutineService _service;
        private readonly ICelebrityService _celebService;

        public WorkoutRoutinePageController(IWorkoutRoutineService service, ICelebrityService celebService)
        {
            _service = service;
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> Edit(int id, WorkoutRoutineDTO dto)
        {
            if (!ModelState.IsValid)
            {
                await LoadCelebrityDropdown();
        ...
        // ✅ Fix: wrap DTO list in SelectList
